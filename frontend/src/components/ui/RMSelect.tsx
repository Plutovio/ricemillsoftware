  ...
  ({ label, error, options, placeholder = 'Select...', className = '', id, ...props }, ref) => {
    ...
    return (
      <div className="flex flex-col gap-1">
        {label && (
          <label htmlFor={selectId} className="text-xs font-medium text-gray-600 dark:text-slate-400 uppercase tracking-wide">
            {label}
          </label>
        )}
        <select
          ...
          {...props}
        >
          <option value="" className="bg-white dark:bg-slate-800 text-gray-900 dark:text-white">{placeholder}</option>
          {options.map((opt) => (
